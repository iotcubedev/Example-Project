  void Compute(OpKernelContext* ctx_modified) override {
    try {
      const Tensor& input = ctx_modified->input(kInputTensorIndex);
      OP_REQUIRES(
          ctx_modified, input.dims() == 4,
          errors::InvalidArgument("Current RequantizePerChannel operator"
                                  "supports 4D tensors only."));

      const Tensor& input_min_vec = ctx_modified->input(kInputMinVecIndex);
      size_t depth = input_min_vec.NumElements();
      float* input_min_vec_data = (float*)const_cast<void*>(
          static_cast<const void*>(input_min_vec.flat<float>().data()));

      const Tensor& input_max_vec = ctx_modified->input(kInputMaxVecIndex);
      OP_REQUIRES(
          ctx_modified, input_max_vec.NumElements() == depth,
          errors::InvalidArgument("input_max has incorrect size, expected ",
                                  depth, " was ", input_max_vec.NumElements()));
      float* input_max_vec_data = (float*)const_cast<void*>(
          static_cast<const void*>(input_max_vec.flat<float>().data()));

      const Tensor& input_requested_min = ctx_modified->input(this->kRequestMinIndex);
      OP_REQUIRES(
          ctx_modified, input_requested_min.NumElements() == 1,
          errors::InvalidArgument("requested_output_min must be a scalar"));
      const float input_requested_min_float =
          input_requested_min.flat<float>()(0);

      const Tensor& input_requested_max = ctx_modified->input(this->kRequestMaxIndex);
      OP_REQUIRES(
          ctx_modified, input_requested_min.NumElements() == 1,
      ...
      if (out_type_ == DT_QINT8) {
        OP_REQUIRES(ctx_modified, input_requested_min_float < 0.0f,
      ...
      OP_REQUIRES_OK(ctx_modified, ctx_modified->allocate_output(kOutputTensorIndex,
                                               input.shape(), &output));
      ...
      MklDnnThreadPool eigen_tp(ctx_modified);
      ...
      OP_REQUIRES_OK(ctx_modified,
                     ctx_modified->allocate_output(kOutputMinIndex, {}, &output_min));
      OP_REQUIRES_OK(ctx_modified,
                     ctx_modified->allocate_output(kOutputMaxIndex, {}, &output_max));
      ...
      OP_REQUIRES_OK(
          ctx_modified, errors::Aborted("Operation received an exception:", error_msg));
    }
  }